...
export function success<T>(req: Request, res: Response, data: T, status = 200) {
  const body: ApiSuccess<T> = { success: true, data, requestId: (req as any).requestId };
  res.status(status).json(body);
}

export function error(req: Request, res: Response, code: string, message: string, details?: any, status = 400, retryable = false) {
  const body: ApiError = { success: false, code, message, details, requestId: (req as any).requestId, retryable };
  res.status(status).json(body);
}

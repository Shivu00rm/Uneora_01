...
export class APIIntegrationsService {
  ...
  async updateIntegration(
    ...
    updates: Partial<Omit<APIIntegration, 'id' | 'organization_id' | 'created_at' | 'updated_at'>>
  ): Promise<APIIntegration> {
    const { data, error } = await supabase
      .from('api_integrations')
      .update(updates)
  ...
  async syncIntegration(integrationId: string): Promise<{ success: boolean; message: string; data?: any }> {
    try {
      const { data: integration, error } = await supabase
        .from('api_integrations')
        .select('*')
  ...
  // Validate credentials based on service type
  private validateCredentials(serviceName: string, credentials: IntegrationCredentials): void {
    switch (serviceName.toLowerCase()) {
      case 'shopify':
        if (!credentials.shop_domain || !credentials.access_token) {
          throw new Error('Shopify integration requires shop_domain and access_token');
        }
        break;
      case 'woocommerce':
        if (!credentials.consumer_key || !credentials.consumer_secret || !credentials.store_url) {
          throw new Error('WooCommerce integration requires consumer_key, consumer_secret, and store_url');
        }
        break;
      case 'razorpay':
        if (!credentials.key_id || !credentials.key_secret) {
          throw new Error('Razorpay integration requires key_id and key_secret');
        }
        break;
      case 'stripe':
  ...
  private async performIntegrationTest(integration: APIIntegration): Promise<{ success: boolean; message: string }> {
  ...
  // Perform data sync
  private async performDataSync(integration: APIIntegration): Promise<{ success: boolean; message: string; data?: any }> {
    switch (integration.service_name.toLowerCase()) {
      case 'shopify':
  ...
  private async testShopifyIntegration(integration: APIIntegration): Promise<{ success: boolean; message: string }> {
    try {
  ...
  private async syncShopifyData(integration: APIIntegration): Promise<{ success: boolean; message: string; data?: any }> {
    try {
      const { shop_domain, access_token } = integration.credentials;
      
      // Fetch products from Shopify
      const response = await fetch(`https://${shop_domain}/admin/api/2023-10/products.json?limit=50`, {
        headers: {
          'X-Shopify-Access-Token': access_token,
          'Content-Type': 'application/json'
        }
  ...
  private async testWooCommerceIntegration(integration: APIIntegration): Promise<{ success: boolean; message: string }> {
    try {
      const { consumer_key, consumer_secret, store_url } = integration.credentials;
  ...
  private async syncWooCommerceData(integration: APIIntegration): Promise<{ success: boolean; message: string; data?: any }> {
    try {
      const { consumer_key, consumer_secret, store_url } = integration.credentials;
      const auth = btoa(`${consumer_key}:${consumer_secret}`);
      
      // Fetch products from WooCommerce
      const response = await fetch(`${store_url}/wp-json/wc/v3/products?per_page=50`, {
        headers: {
          'Authorization': `Basic ${auth}`,
          'Content-Type': 'application/json'
        }
  ...
  private async testRazorpayIntegration(integration: APIIntegration): Promise<{ success: boolean; message: string }> {
    try {
...
export function useAPIIntegrations() {
  ...
  return {
    ...
    createIntegration: (serviceName: string, serviceType: string, credentials: IntegrationCredentials, config?: IntegrationConfig) =>
      getService().createIntegration(serviceName, serviceType, credentials, config),

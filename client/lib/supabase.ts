...
if (hasValidSupabaseConfig) {
  ...
  console.warn('🔄 Using mock Supabase client - set real VITE_SUPABASE_URL and VITE_SUPABASE_ANON_KEY for production')
  ...
  supabase = {
    ...
    from: (table: string) => ({
      select: () => ({
        eq: () => ({
          single: async () => ({ data: null, error: { message: 'Mock database - configure Supabase for real data' } })
        })
      }),
      insert: () => ({
        select: () => ({

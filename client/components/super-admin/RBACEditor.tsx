...
export default function RBACEditor() {
  ...
  const [matrix, setMatrix] = useState<Record<string, Record<string, boolean>>>(
    () => {
      ...
      m.inventory.view = true;
      m.analytics.view = true;
      return m;
    },
  );
  ...
  const save = async () => {
    await fetch("/api/admin/rbac/update", {
      method: "POST",
            ...
            <div className="flex items-center gap-2">
              ...
              {selectedOrg && (
                <span className="text-xs text-muted-foreground ml-2">
                  Plan:{" "}
                  <strong className="uppercase">{selectedOrg.plan}</strong>
                </span>
              )}
            </div>
            {selectedOrg && (
              <Button
                variant="outline"
                size="sm"
                onClick={() => applyPlanDefaults(selectedOrg.id)}
              >
                Apply Plan Defaults
              </Button>
            )}
          ...
          {selectedOrg && (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
              {ALL_FEATURES.map((f) => {
                ...
                const inPlan = PLAN_FEATURES[selectedOrg.plan]?.includes(
                  f as Feature,
                );
                return (
                  <label
                  ...
                  >
                    <input
                      ...
                      checked={enabled}
                      onChange={(e) =>
                        toggleFeature(f as Feature, e.target.checked)
                      }
                    />

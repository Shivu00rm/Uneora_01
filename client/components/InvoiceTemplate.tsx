...
import { Building2, Mail, Phone, Globe, Calendar, CreditCard } from "lucide-react";
    ...
    switch (plan) {
      case "starter":
        return { 
          name: "Starter Plan", 
          price: 999, 
          features: ["Inventory Management", "GST-Compliant POS", "Basic Reports", "WhatsApp Alerts"]
        };
      case "growth":
        return { 
          name: "Growth Plan", 
          price: 2999, 
          features: ["Everything in Starter", "E-commerce Sync", "Smart Automation", "Multi-user Access"]
        };
      case "pro":
        return { 
          name: "Pro Plan", 
          price: 9999, 
          features: ["Everything in Growth", "AI Analytics", "MSME Consulting", "Priority Support"]
        };
      case "enterprise":
        return { 
      ...
      <div className="flex justify-between items-start mb-8">
        ...
        <div className="text-right">
          ...
          <div className="text-sm space-y-1">
            ...
            <div><strong>Issue Date:</strong> {new Date(issueDate).toLocaleDateString('en-IN')}</div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
        ...
        <div>
          ...
          <div className="space-y-2 text-sm">
            <div><strong>Current Plan:</strong> {planDetails.name}</div>
            <div><strong>Active Users:</strong> {organization.users}</div>
            <div><strong>Billing Period:</strong> Monthly</div>
            <div><strong>Service Status:</strong> <Badge variant="default">Active</Badge></div>
          </div>
        </div>
      </div>
        ...
        <div className="border rounded-lg overflow-hidden">
          <table className="w-full">
            ...
            <tbody className="divide-y">
              <tr>
                <td className="p-4">
                  <div className="font-medium">{planDetails.name} - Monthly Subscription</div>
                  <div className="text-sm text-gray-600 mt-1">
                    Includes: {planDetails.features.slice(0, 2).join(", ")}
                    {planDetails.features.length > 2 && ` + ${planDetails.features.length - 2} more features`}
                  </div>
                </td>
                <td className="p-4 text-center text-sm text-gray-600">
                  {new Date(issueDate).toLocaleDateString('en-IN', { month: 'short', year: 'numeric' })}
                </td>
                <td className="p-4 text-right font-medium">₹{subtotal.toLocaleString()}</td>
              </tr>
            </tbody>
          </table>
        </div>
